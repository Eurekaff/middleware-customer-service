...
- 包含“怎么购买、多少钱、功能” -> 售前咨询；
- 包含“商品、产品、课程、服务、套餐、适合、介绍” -> 商品咨询；
- 包含“你好、您好、在吗、早上好、下午好、晚上好、hello、hi” -> 问候闲聊；
- 纯数字、过短且不可识别的内容 -> 无效输入；
- 其他 -> 一般问题。
...
当前课程演示版本中，Worker 通过 `worker/services/mcp_client.py` 复用 `mcp_server/tools/customer_service_tools.py` 中的工具函数，保证本地链路稳定可运行。`mcp_server/server.py` 保留真实 MCP Server 入口，后续可以把 `mcp_client.py` 替换为标准 MCP Client 调用。
